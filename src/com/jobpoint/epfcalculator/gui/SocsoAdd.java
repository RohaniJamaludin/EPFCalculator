...
public class SocsoAdd implements ActionListener{
	...
	public SocsoAdd(boolean isSixty) {
		this.isSixty = isSixty;
		initialize();
	}
	
	private void initialize() {
		SocsoMain.footerPanel.getComponent(0).setEnabled(false); 
		...
		dialog = new JDialog(SocsoMain.frame,"Add SOCSO Contribution Rate " + s, true);
		dialog.addWindowListener(new WindowAdapter() {
			public void  windowClosed(WindowEvent event) {
				System.out.println("Closed");
				SocsoMain.footerPanel.getComponent(0).setEnabled(true);
			}
		});
		...
		// TODO Auto-generated method stub
				...
				if(event.getSource().equals(saveButton)) {
					...
					if(!error) {
						...
						if(socsoController.saveSocso(isSixty)) {
							dialog.dispose();
							JOptionPane.showMessageDialog(null, "New data has been added!");
						}else {
							JOptionPane.showMessageDialog(null, "New data cannot be added!");
						}
