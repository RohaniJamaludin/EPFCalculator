...
public class SipEdit implements ActionListener{
	...
	public SipEdit (Object[] data, int index) {
		this.index = index;
		this.data = data;
		initialize();
	}
	...
	private void initialize() {
		...
		dialog = new JDialog(SipMain.frame,"Edit EPF Contribution Rate", true);
		dialog.addWindowListener(new WindowAdapter() {
			public void  windowClosed(WindowEvent event) {
				System.out.println("Closed");
				SipMain.footerPanel.getComponent(1).setEnabled(true);
			}
		});
	...
	public void actionPerformed(ActionEvent event) {
		...
		if(event.getSource().equals(saveButton)) {
			...
			if(!error) {
				...
				if(sipController.updateSip(id, index)) {
					dialog.dispose();
